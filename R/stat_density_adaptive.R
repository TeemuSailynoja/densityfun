#' Stat for adaptive KDE
#'
#' @param bw The smoothing bandwidth to be used for the reference density.
...
#' @export
#' @rdname geom_density_adaptive
#' @importFrom ggplot2 layer
stat_density_adaptive <- function(mapping = NULL,
                                  data = NULL,
                                  geom = "area",
                                  position = "stack",
                                  ...,
...
#' @importFrom ggplot2 flipped_names
...
compute_density_adaptive <-
  function(x,
    ...
    if (any(is.finite(bounds))) {
      dens <-
        list(x = seq(from - 3 * bw0, to + 3 * bw0, length.out = n),
             y = denfun(seq(from - 3 * bw0, to + 3 * bw0, length.out = n)))
      ...
      dens <- list(x = seq(from, to, length.out = n),
                   y = denfun(seq(from, to, length.out = n)))
    }
...
#' @rdname geom_density_adaptive
#' @export
geom_density_adaptive <- function(mapping = NULL,
                                  data = NULL,
                                  stat = "density_adaptive",
                                  position = "identity",
                                  ...,
                                  na.rm = FALSE,
                                  orientation = NA,
                                  ...
                                  outline.type = "upper") {
  outline.type <-
    rlang::arg_match0(outline.type, c("both", "upper", "lower", "full"))

  ggplot2::layer(
    data = data,
    mapping = mapping,
    stat = stat,
    geom = GeomDensityAdaptive,
    position = position,
    show.legend = show.legend,
    inherit.aes = inherit.aes,
    params = rlang::list2(
...
# helpers for plots --------------- from ggplot2
